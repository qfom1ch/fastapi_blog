...
from app.blog.schemas import ShowPost, PostCreate, UpdatedPostResponse, UpdatePostRequest
from app.user import security
from app.user.models import User
from app.user.services import _get_user_by_id, _check_user_permissions
from db.session import get_db
from fastapi_pagination import Page, paginate
from app.blog.services import (_create_post,
                               _get_post_by_id,
                               _get_post_by_slug,
                               _get_post_by_title,
                               _get_posts_by_user_id, _get_all_posts, _update_post, _delete_post)
...
@post_router.get("/user", response_model=Page[ShowPost], tags=['Posts'])
async def get_posts_by_user_id(user_id: int, db_session: AsyncSession = Depends(get_db)) -> Page[ShowPost]:
...
async def get_all_posts(db_session: AsyncSession = Depends(get_db)) -> Page[ShowPost]:
...
async def create_post(post: PostCreate,
                      ...
                      current_user: User = Depends(security.get_current_user_from_token)) -> ShowPost:
...
async def update_post_by_id(post_id: int,
                            ...
                            db_session: AsyncSession = Depends(get_db),
                            current_user: User = Depends(security.get_current_user_from_token)) -> UpdatedPostResponse:
    ...
    if author_id != current_user.id:
        ...
        if not _check_user_permissions(target_user=user_author, current_user=current_user):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Forbidden. Not enough rights.")
    updated_post = await _update_post(post_id, updated_post_params, db_session)
    return updated_post


@post_router.delete("/posts", status_code=status.HTTP_204_NO_CONTENT, tags=['Posts'])
async def delete_post(
        post_id: int,
        db_session: AsyncSession = Depends(get_db),
        current_user: User = Depends(security.get_current_user_from_token)) -> Response:
    post = await _get_post_by_id(post_id, db_session)
    if post is None:
